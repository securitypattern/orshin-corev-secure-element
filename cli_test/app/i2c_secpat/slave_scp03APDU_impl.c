...
/*** BEGIN Static keys. ***/
uint8_t ENC_key[AES_KEY_LEN] = {0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0x00, 0x01}; /* SE static ENC key. */
uint8_t MAC_key[AES_KEY_LEN] = {0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0x00, 0x02}; /* SE static MAC key. */
/*** END Static keys. ***/
...
void set_rec_APDU_message(uint8_t *ptrRecAPDU){
	if(ptrRecAPDU[4] != 0x00){
		...
		memcpy(scp03_sess_ctxt.rx_APDU_message, ptrRecAPDU, scp03_sess_ctxt.rx_APDU_header_len + scp03_sess_ctxt.rx_APDU_data_len);
	}
	else{
...
void scp03APDU_construct_resp_APDU_message(uint8_t *ptrAPDUCommand, uint8_t *ptrAPDUResponse, uint8_t *ptrAPDUResponseLen){
	...
	if(scp03_sess_ctxt.rx_APDU_command_type <= EXTERNAL_AUTHENTICATE){ /* Secure channel session not yet established. */
		...
		prepare_response_APDU(ptrAPDUCommand, ptrAPDUResponse, ptrAPDUResponseLen);
	}
}
...
void initialize_update_response(uint8_t *ptrAPDUResponse, uint8_t *ptrAPDUResponseLen){
	...
	scp03_sess_ctxt.card_challenge[0] = 0x44; scp03_sess_ctxt.card_challenge[1] = 0x21; scp03_sess_ctxt.card_challenge[2] = 0x35; scp03_sess_ctxt.card_challenge[3] = 0x55;
	scp03_sess_ctxt.card_challenge[4] = 0x52; scp03_sess_ctxt.card_challenge[5] = 0x00; scp03_sess_ctxt.card_challenge[6] = 0x10; scp03_sess_ctxt.card_challenge[7] = 0x11;
	memcpy(&ptrAPDUResponse[i], scp03_sess_ctxt.card_challenge, CARD_CHALLENGE_LEN);
	i += CARD_CHALLENGE_LEN;
	/*** END Add card challenge. ***/

	memcpy(scp03_sess_ctxt.host_challenge, &scp03_sess_ctxt.rx_APDU_message[scp03_sess_ctxt.rx_APDU_header_len], HOST_CHALLENGE_LEN); /* Retrieve host challenge. */
	...
	memcpy(&ptrAPDUResponse[i], card_cryptogram_full_len, (CARD_CRYPTOGRAM_FULL_LEN / 2));
...
void external_authenticate_response(uint8_t *ptrAPDUResponse, uint8_t *ptrAPDUResponseLen){
	...
	scp03aux_verify_host_cryptogram(&scp03_sess_ctxt, &isTrue_verify_host_cryptogram); /* Verify the received host cryptogram. */
...
void prepare_response_APDU(uint8_t *ptrAPDUCommand, uint8_t *ptrAPDUResponse, uint8_t *ptrAPDUResponseLen){
	...
	if(isTrue_verify_MAC){ /* Command processing OK. */
		...
		if(command_data_proper_len > 0 && (scp03_sess_ctxt.curr_sec_level == C_DEC_C_MAC || scp03_sess_ctxt.curr_sec_level >= C_DEC_C_MAC_R_MAC)){
			scp03aux_decrypt_com_APDU(&scp03_sess_ctxt, command_data_dec, &command_data_proper_len);
		}
		else{ /* Data received from master is not encrypted. */
			memcpy(command_data_dec, &scp03_sess_ctxt.rx_APDU_message[scp03_sess_ctxt.rx_APDU_header_len], command_data_proper_len);
		}
		/*** END Check if received data is encrypted ***/

		switch(scp03_sess_ctxt.rx_APDU_command_type){
		case GET_VERSION:
			//CLI_printf("Gen GET Version response BEGIN\n");
			get_version_response(response_data, &response_data_len);
			//CLI_printf("Gen GET Version response END\n");
			break;
		case CHECK_OBJECT_EXISTS:
			check_object_exists_response(response_data, &response_data_len, command_data_dec);
			break;
		case WRITE_ECKEY:
			write_eckey_response(response_data, &response_data_len, command_data_dec);
			break;
		case DELETE_SECURE_OBECT:
			delete_secure_object_response(response_data, &response_data_len, command_data_dec);
			break;
		case READ_OBJECT:
			read_object_response(response_data, &response_data_len, command_data_dec);
			break;
		case ECHO:
		...
		scp03aux_apply_session_security_requirements_resp_APDU(&scp03_sess_ctxt, ptrAPDUResponse, ptrAPDUResponseLen, response_data, response_data_len, SW1, SW2); /* Apply the security requirements of the current SCP session. */
...
void check_object_exists_response(uint8_t *ptrResponseData, uint8_t *ptrResponseDataLen, uint8_t *ptrCommandDataDec){
	...
	*ptrResponseDataLen = 3; /* Tag (1 byte), actual response data length (1 byte), actual response (1 byte). */
	...
	for(uint8_t i = 0; i < MAX_SEC_OBJ_NUM; i++){
		if(memcmp(sec_objs[i].object_identifier, &ptrCommandDataDec[2], ptrCommandDataDec[1]) == 0){
...
void write_eckey_response(uint8_t *ptrResponseData, uint8_t *ptrResponseDataLen, uint8_t *ptrCommandDataDec){
	...
	if(j < MAX_SEC_OBJ_NUM){ /* Implies that the secure object does not exist AND there is an available cell to store the new secure object. The value of variable 'j' indicates the index. */
		memcpy(sec_objs[j].object_identifier, &ptrCommandDataDec[2], ptrCommandDataDec[1]);
		sec_objs[j].authentication_indicator = 1; /* SET, because it is an authenticated object in this case. */
		key_pair_offset = 1 /* TAG */ + 1 /* length */ + ptrCommandDataDec[1] + 1 /* TAG */ + 1 /* length */ + ptrCommandDataDec[4];
		memcpy(sec_objs[j].data, &ptrCommandDataDec[key_pair_offset], scp03_sess_ctxt.rx_APDU_data_len - key_pair_offset);
	}
