 ...
 *  [!] NOTE: (very slight) modifications have been carried out to the present file, with respect to the original (slight modifications on preprocessor directives).
 *
...
static inline const mbedtls_cipher_base_t* mbedtls_cipher_get_base(
	...
	const mbedtls_cipher_base_t *p =
			mbedtls_cipher_base_lookup_table[info->base_idx];
	return p;
}
...
const mbedtls_cipher_info_t* mbedtls_cipher_info_from_values(
		const mbedtls_cipher_id_t cipher_id, int key_bitlen,
		const mbedtls_cipher_mode_t mode) {
	const mbedtls_cipher_definition_t *def;

	for (def = mbedtls_cipher_definitions; def->info != NULL; def++) {
		if (mbedtls_cipher_get_base(def->info)->cipher == cipher_id
				&& mbedtls_cipher_info_get_key_bitlen(def->info)
						== (unsigned) key_bitlen && def->info->mode == mode) {
			return def->info;
		}
	}
...
void mbedtls_cipher_free(mbedtls_cipher_context_t *ctx) {
	...
	if (ctx->cipher_ctx) {
		mbedtls_cipher_get_base(ctx->cipher_info)->ctx_free_func(
				ctx->cipher_ctx);
	}
...
int mbedtls_cipher_setup(mbedtls_cipher_context_t *ctx,
	...
	memset(ctx, 0, sizeof(mbedtls_cipher_context_t));
	const mbedtls_cipher_base_t* cipher_base = mbedtls_cipher_get_base(cipher_info);
	ctx->cipher_ctx = cipher_base->ctx_alloc_func();
	if (NULL == ctx->cipher_ctx) {
...
int mbedtls_cipher_setkey(mbedtls_cipher_context_t *ctx,
		const unsigned char *key, int key_bitlen,
	...
	if ((ctx->cipher_info->flags & MBEDTLS_CIPHER_VARIABLE_KEY_LEN) == 0
			&& (int) mbedtls_cipher_info_get_key_bitlen(ctx->cipher_info)
					!= key_bitlen) {
		return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
	}
	...
	if (MBEDTLS_ENCRYPT == operation
			|| MBEDTLS_MODE_CFB
					== ((mbedtls_cipher_mode_t) ctx->cipher_info->mode)
			|| MBEDTLS_MODE_OFB
					== ((mbedtls_cipher_mode_t) ctx->cipher_info->mode)
			|| MBEDTLS_MODE_CTR
					== ((mbedtls_cipher_mode_t) ctx->cipher_info->mode)) {
		return mbedtls_cipher_get_base(ctx->cipher_info)->setkey_enc_func(
				ctx->cipher_ctx, key, ctx->key_bitlen);
	}

	if (MBEDTLS_DECRYPT == operation) {
		return mbedtls_cipher_get_base(ctx->cipher_info)->setkey_dec_func(
				ctx->cipher_ctx, key, ctx->key_bitlen);
	}
...
#if defined(MBEDTLS_CHACHA20_C)
	if (((mbedtls_cipher_type_t) ctx->cipher_info->type)
		...
		if (0
				!= mbedtls_chacha20_starts(
						(mbedtls_chacha20_context*) ctx->cipher_ctx, iv, 0U)) { /* Initial counter value */
			return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
		}
	}
#if defined(MBEDTLS_CHACHAPOLY_C)
	if (((mbedtls_cipher_type_t) ctx->cipher_info->type)
			== MBEDTLS_CIPHER_CHACHA20_POLY1305 && iv_len != 12) {
		return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
	}
#endif
#endif
...
#if defined(MBEDTLS_CCM_C)
	if (MBEDTLS_MODE_CCM_STAR_NO_TAG
			== ((mbedtls_cipher_mode_t) ctx->cipher_info->mode)) {
		int set_lengths_result;
		int ccm_star_mode;

		set_lengths_result = mbedtls_ccm_set_lengths(
				(mbedtls_ccm_context*) ctx->cipher_ctx, 0, 0, 0);
...
#if defined(MBEDTLS_GCM_C)
	if (MBEDTLS_MODE_GCM == ((mbedtls_cipher_mode_t) ctx->cipher_info->mode)) {
		return mbedtls_gcm_update_ad((mbedtls_gcm_context*) ctx->cipher_ctx, ad,
				ad_len);
	}
#endif

#if defined(MBEDTLS_CHACHAPOLY_C)
	if (MBEDTLS_CIPHER_CHACHA20_POLY1305
			== ((mbedtls_cipher_type_t) ctx->cipher_info->type)) {
		int result;
		mbedtls_chachapoly_mode_t mode;

		mode = (ctx->operation == MBEDTLS_ENCRYPT) ?
				MBEDTLS_CHACHAPOLY_ENCRYPT : MBEDTLS_CHACHAPOLY_DECRYPT;
	...
	*olen = 0;
	...
	if (((mbedtls_cipher_mode_t) ctx->cipher_info->mode) == MBEDTLS_MODE_ECB) {
		...
		*olen = ilen;
		...
		if (0
				!= (ret = mbedtls_cipher_get_base(ctx->cipher_info)->ecb_func(
	...
	if (((mbedtls_cipher_mode_t) ctx->cipher_info->mode) == MBEDTLS_MODE_GCM) {
		return mbedtls_gcm_update((mbedtls_gcm_context*) ctx->cipher_ctx, input,
				ilen, output, ilen, olen);
	}
#endif

#if defined(MBEDTLS_CCM_C)
	if (((mbedtls_cipher_mode_t) ctx->cipher_info->mode)
			== MBEDTLS_MODE_CCM_STAR_NO_TAG) {
		return mbedtls_ccm_update((mbedtls_ccm_context*) ctx->cipher_ctx, input,
				ilen, output, ilen, olen);
	}
#endif
	...
	if (((mbedtls_cipher_mode_t) ctx->cipher_info->mode) == MBEDTLS_MODE_CBC) {
		...
		if (0 != ctx->unprocessed_len) {
			...
			if (0
					!= (ret =
							mbedtls_cipher_get_base(ctx->cipher_info)->cbc_func(
									ctx->cipher_ctx, ctx->operation, block_size,
	...
	if (((mbedtls_cipher_mode_t) ctx->cipher_info->mode) == MBEDTLS_MODE_CFB) {
		if (0
				!= (ret = mbedtls_cipher_get_base(ctx->cipher_info)->cfb_func(
						ctx->cipher_ctx, ctx->operation, ilen,
	...
	if (((mbedtls_cipher_mode_t) ctx->cipher_info->mode) == MBEDTLS_MODE_OFB) {
		if (0
				!= (ret = mbedtls_cipher_get_base(ctx->cipher_info)->ofb_func(
	...
	if (((mbedtls_cipher_mode_t) ctx->cipher_info->mode) == MBEDTLS_MODE_CTR) {
		if (0
				!= (ret = mbedtls_cipher_get_base(ctx->cipher_info)->ctr_func(
						ctx->cipher_ctx, ilen, &ctx->unprocessed_len, ctx->iv,
	...
	if (((mbedtls_cipher_mode_t) ctx->cipher_info->mode) == MBEDTLS_MODE_XTS) {
		...
		ret = mbedtls_cipher_get_base(ctx->cipher_info)->xts_func(
	...
	if (((mbedtls_cipher_mode_t) ctx->cipher_info->mode)
		...
		if (0
				!= (ret =
						mbedtls_cipher_get_base(ctx->cipher_info)->stream_func(
...
static int get_pkcs_padding(unsigned char *input, size_t input_len,
	...
	*data_len = input_len - padding_len;
	...
	for (i = 0; i < input_len; i++) {
		...
		mbedtls_ct_condition_t different = mbedtls_ct_uint_ne(input[i],
				padding_len);
		bad = mbedtls_ct_bool_or(bad,
				mbedtls_ct_bool_and(in_padding, different));
	}
 ...
 */
static void add_one_and_zeros_padding(unsigned char *output, size_t output_len,
		size_t data_len) {
	size_t padding_len = output_len - data_len;
	unsigned char i = 0;
...
static int get_one_and_zeros_padding(unsigned char *input, size_t input_len,
	...
	*data_len = 0;
	...
	for (ptrdiff_t i = (ptrdiff_t) (input_len) - 1; i >= 0; i--) {
		...
		mbedtls_ct_condition_t hit_first_nonzero = mbedtls_ct_bool_and(
				is_nonzero, in_padding);

		*data_len = mbedtls_ct_size_if(hit_first_nonzero, i, *data_len);

		bad = mbedtls_ct_bool_if(hit_first_nonzero,
				mbedtls_ct_uint_ne(input[i], 0x80), bad);

		in_padding = mbedtls_ct_bool_and(in_padding,
				mbedtls_ct_bool_not(is_nonzero));
	}
 ...
 */
static void add_zeros_and_len_padding(unsigned char *output, size_t output_len,
		size_t data_len) {
	size_t padding_len = output_len - data_len;
	unsigned char i = 0;
...
static int get_zeros_and_len_padding(unsigned char *input, size_t input_len,
	...
	*data_len = input_len - padding_len;
	...
	for (i = 0; i < input_len - 1; i++) {
		...
		nonzero_pad_byte = mbedtls_ct_bool_if_else_0(is_padding,
 ...
 */
static void add_zeros_padding(unsigned char *output, size_t output_len,
		size_t data_len) {
...
static int get_zeros_padding(unsigned char *input, size_t input_len,
	...
	*data_len = 0;
	for (i = input_len; i > 0; i--) {
		...
		*data_len = mbedtls_ct_size_if(mbedtls_ct_bool_ne(done, prev_done), i,
				*data_len);
	}
...
int mbedtls_cipher_finish(mbedtls_cipher_context_t *ctx, unsigned char *output,
		size_t *olen) {
	if (ctx->cipher_info == NULL) {
		return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
	}
	...
	if (MBEDTLS_MODE_CBC == ((mbedtls_cipher_mode_t) ctx->cipher_info->mode)) {
		...
		if (MBEDTLS_ENCRYPT == ctx->operation) {
			...
			ctx->add_padding(ctx->unprocessed_data,
					mbedtls_cipher_get_iv_size(ctx), ctx->unprocessed_len);
		...
		/* cipher block */
		if (0
				!= (ret = mbedtls_cipher_get_base(ctx->cipher_info)->cbc_func(
...
int mbedtls_cipher_write_tag(mbedtls_cipher_context_t *ctx, unsigned char *tag,
	...
	if (MBEDTLS_MODE_GCM == ((mbedtls_cipher_mode_t) ctx->cipher_info->mode)) {
		...
		NULL, 0, &output_length, tag, tag_len);
	}
#endif
...
int mbedtls_cipher_check_tag(mbedtls_cipher_context_t *ctx,
	...
	if (MBEDTLS_MODE_GCM == ((mbedtls_cipher_mode_t) ctx->cipher_info->mode)) {
		...
		if (0
				!= (ret = mbedtls_gcm_finish(
						...
						NULL, 0, &output_length, check_tag, tag_len))) {
			return ret;
		}
	...
	exit: mbedtls_platform_zeroize(check_tag, tag_len);
	return ret;
}
#endif /* MBEDTLS_GCM_C || MBEDTLS_CHACHAPOLY_C */
 ...
 */
int mbedtls_cipher_crypt(mbedtls_cipher_context_t *ctx, const unsigned char *iv,
		size_t iv_len, const unsigned char *input, size_t ilen,
...
static int mbedtls_cipher_aead_encrypt(mbedtls_cipher_context_t *ctx,
		const unsigned char *iv, size_t iv_len, const unsigned char *ad,
		size_t ad_len, const unsigned char *input, size_t ilen,
	...
	if (MBEDTLS_CIPHER_CHACHA20_POLY1305
		...
		*olen = ilen;
		return mbedtls_chachapoly_encrypt_and_tag(ctx->cipher_ctx, ilen, iv, ad,
				ad_len, input, output, tag);
	}
#endif /* MBEDTLS_CHACHAPOLY_C */
...
static int mbedtls_cipher_aead_decrypt(mbedtls_cipher_context_t *ctx,
		const unsigned char *iv, size_t iv_len, const unsigned char *ad,
		size_t ad_len, const unsigned char *input, size_t ilen,
		unsigned char *output, size_t *olen, const unsigned char *tag,
		size_t tag_len) {
#if defined(MBEDTLS_USE_PSA_CRYPTO) && !defined(MBEDTLS_DEPRECATED_REMOVED)
    if (ctx->psa_enabled == 1) {
        /* As in the non-PSA case, we don't check that
...
int mbedtls_cipher_auth_encrypt_ext(mbedtls_cipher_context_t *ctx,
	...
	int ret = mbedtls_cipher_aead_encrypt(ctx, iv, iv_len, ad, ad_len, input,
			ilen, output, olen, output + ilen, tag_len);
	*olen += tag_len;
	return ret;
#else
    return MBEDTLS_ERR_CIPHER_FEATURE_UNAVAILABLE;
...
int mbedtls_cipher_auth_decrypt_ext(mbedtls_cipher_context_t *ctx,
	...
	return mbedtls_cipher_aead_decrypt(ctx, iv, iv_len, ad, ad_len, input,
			ilen - tag_len, output, olen, input + ilen - tag_len, tag_len);
#else
    return MBEDTLS_ERR_CIPHER_FEATURE_UNAVAILABLE;
#endif /* MBEDTLS_CIPHER_MODE_AEAD */
}
